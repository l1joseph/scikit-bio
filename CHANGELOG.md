...
* Added `optspace` (OptSpace low-rank matrix completion) to `skbio.stats.ordination` for recovering a low-rank matrix from a small set of observed entries (with missing entries marked as `NaN`). This is the matrix-completion core of robust Aitchison PCA (RPCA) of sparse compositional data. The default `"cython"` engine is a pure NumPy/SciPy implementation requiring no compiled extension; an optional `"numba"` engine JIT-compiles the fusable element-wise and reduction kernels (reproducing the default-engine result to within ~1e-8) and is selected via the `engine=` parameter (see `skbio.set_config`). The algorithm and its NumPy implementation were adapted from the `gemelli` package (Modified BSD License, https://github.com/biocore/gemelli), itself a port of Sewoong Oh's MATLAB OptSpace code; a copy of the gemelli license is included in `licenses/gemelli.txt`.
